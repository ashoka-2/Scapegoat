...
const AdminDashboard = () => {
  ...
  const maxRevenueInChart = Math.max(...chartData.map((c) => c.revenue || 1), 100);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-surface border border-border-theme p-5 rounded-2xl space-y-3 shadow-sm relative overflow-hidden group hover:border-accent transition">
          ...
          <div className="flex items-center justify-between text-[11px] font-semibold text-foreground/60 pt-1 border-t border-border-theme/40">
            ...
            <span className="text-emerald-500 font-bold flex items-center"><i className="ri-arrow-up-line" /> Verified</span>
          </div>
        </div>
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 border-b border-border-theme/60 pb-4">
          <div>
            <h2 className="text-sm font-black uppercase tracking-wider text-foreground flex items-center gap-2">
              <i className="ri-calendar-event-line text-accent" /> Revenue Track & Real Order Performance
            </h2>
          ...
          <div className="flex flex-wrap items-center gap-2 w-full lg:w-auto">
            <div className="flex items-center bg-background border border-border-theme rounded-xl p-1 gap-1">
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition cursor-pointer ${
                  timeframe === "daily" ? "bg-accent text-accent-content" : "text-foreground/70 hover:text-foreground"
                }`}
              >
                Daily
              </button>
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition cursor-pointer ${
                  timeframe === "monthly" ? "bg-accent text-accent-content" : "text-foreground/70 hover:text-foreground"
                }`}
              >
                Monthly
              </button>
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition cursor-pointer ${
                  timeframe === "yearly" ? "bg-accent text-accent-content" : "text-foreground/70 hover:text-foreground"
                }`}
              >
                Yearly
              </button>
            ...
            <form onSubmit={handleApplyDateFilter} className="flex items-center gap-2">
              <input
                type="date"
                value={startDate}
        ...
        {chartData.length > 0 ? (
          <div className="h-60 flex items-end justify-between gap-3 pt-6 pb-2 px-2 border-b border-border-theme/50 overflow-x-auto">
            {chartData.map((item, idx) => {
              const heightPercent = Math.max(15, Math.round((item.revenue / maxRevenueInChart) * 100));
              return (
                <div key={idx} className="flex-1 min-w-[50px] flex flex-col items-center gap-2 group h-full justify-end">
          ...
          <div className="py-12 text-center text-xs text-foreground/40 italic bg-background/30 rounded-2xl border border-border-theme/30">
            No completed sales orders recorded for the selected timeframe or date range.
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-surface border border-border-theme rounded-3xl p-6 space-y-4 shadow-sm">
          <div className="flex items-center justify-between border-b border-border-theme/40 pb-3">
            <div>
              <h2 className="text-sm font-black uppercase tracking-wider text-foreground flex items-center gap-2">
                <i className="ri-award-line text-amber-500" /> Best Performing Products (Real Sales)
              </h2>
              <p className="text-[10px] text-foreground/50">Ranked by total quantity sold in completed customer orders</p>
            </div>
            <button
          ...
          <div className="space-y-3">
            {topProducts?.length > 0 ? (
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="font-extrabold text-xs text-foreground truncate max-w-[180px]">{p.title}</p>
                        <p className="text-[10px] text-foreground/50">{p.category?.name || "Category"}</p>
              ...
              <p className="text-xs text-foreground/40 italic py-6 text-center">No product sales recorded yet.</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-surface border border-border-theme rounded-3xl p-6 space-y-4 shadow-sm">
          <div className="flex items-center justify-between border-b border-border-theme/40 pb-3">
            <div>
              <h2 className="text-sm font-black uppercase tracking-wider text-foreground flex items-center gap-2">
                <i className="ri-receipt-line text-accent" /> Recent Transactions
              </h2>
              <p className="text-[10px] text-foreground/50">Click any transaction card to open full printable order receipt</p>
            </div>
            <button
          ...
          <div className="space-y-3">
            {recentOrders?.length > 0 ? (
              ...
              <p className="text-xs text-foreground/40 italic py-6 text-center">No transactions recorded yet.</p>
            )}
          </div>
        </div>
      </div>
          ...
          <div className="bg-surface border border-border-theme rounded-3xl p-6 sm:p-8 max-w-2xl w-full space-y-6 shadow-2xl animate-in fade-in zoom-in-95 max-h-[85vh] flex flex-col">
            <div className="flex items-center justify-between border-b border-border-theme pb-4 shrink-0">
              <div>
                <h2 className="text-lg font-black text-foreground flex items-center gap-2">
                  <i className="ri-user-heart-line text-emerald-500" /> Real-time Online Shoppers ({users?.activeUsersList?.length || 0})
                </h2>
                <p className="text-xs text-foreground/50">Active buyers & sellers browsing the site (Excludes Admins)</p>
              </div>
              <button
            ...
            {analytics?.devices && (
              <div className="flex items-center gap-3 bg-background/60 p-3 rounded-2xl border border-border-theme/40 text-xs font-bold text-foreground/70">
                <span className="text-[10px] font-black uppercase text-accent tracking-wider">Device Breakdown:</span>
                <span className="flex items-center gap-1">
                  <i className="ri-computer-line text-accent" /> Desktop ({analytics.devices.Desktop || 0})
                </span>
                <span className="flex items-center gap-1">
                  <i className="ri-smartphone-line text-emerald-500" /> Mobile ({analytics.devices.Mobile || 0})
                </span>
                <span className="flex items-center gap-1">
                  <i className="ri-tablet-line text-purple-500" /> Tablet ({analytics.devices.Tablet || 0})
                </span>
              </div>
            )}
